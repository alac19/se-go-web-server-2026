...
| 接口                            | 定义                                                                     | 职责                                           | 调用方                  |
| ------------------------------- | ------------------------------------------------------------------------ | ---------------------------------------------- | ----------------------- |
| `LoadBalancer.Next()`           | `func (lb *LoadBalancer) Next() *url.URL`                                | 返回下一个健康后端 URL（轮询），若无可返回 nil | `proxy` 包的 `director` |
| `LoadBalancer.UpdateBackends()` | `func (lb *LoadBalancer) UpdateBackends(urls []*url.URL)`                | 原子替换健康后端列表                           | `health` 包的回调       |
| `LoadBalancer.HealthyCount()`   | `func (lb *LoadBalancer) HealthyCount() int`                             | 返回当前健康后端数量                           | `proxy` 包外层 503 判断 |
| `health.NewChecker()`           | `func NewChecker(..., onHealthy func([]*url.URL)) *Checker`              | 创建健康检查器，注册回调                       | `main.go`               |
| `health.Checker.Start()`        | `func (c *Checker) Start(ctx context.Context)`                           | 启动后台健康检查协程                           | `main.go`               |
| `proxy.NewReverseProxy()`       | `func NewReverseProxy(lb *lb.LoadBalancer, timeoutSec int) http.Handler` | 创建反向代理处理器                             | `main.go`               |
...
## 七、扩展性与维护性
- **模块化**：`config`、`lb`、`health`、`proxy`、`log` 包独立，可单独测试或替换。
